...
TEST_F(detail_test, test_peek) {
    ...
    ASSERT_EQ(magic_bool, tlw::ordered_type_traits<tlw::bool_t>::get(L));

    lua_pushnumber(L, magic_number);
    ASSERT_EQ(magic_number, tlw::ordered_type_traits<tlw::number_t>::get(L));

    lua_pushstring(L, magic_string);
    ASSERT_STREQ(magic_string, tlw::ordered_type_traits<tlw::string_t>::get(L));

    lua_createtable(L, 0, 0);
    auto t_ref = tlw::ordered_type_traits<tlw::table_t>::get(L);
    ASSERT_TRUE(static_cast<bool>(t_ref));

    lua_pushcfunction(L, &noop);
    auto fn_ref = tlw::ordered_type_traits<tlw::function_t>::get(L);
    ...
    *user_data = &magic_user_datum;
    ASSERT_EQ(&magic_user_datum, tlw::ordered_type_traits<tlw::lua_example_t>::get(L));
}
